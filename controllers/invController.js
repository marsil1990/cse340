...
invCont.buildByInventory = async function (req, res, next) {
  ...
  const grid = await utilities.buildByInventoryGrid(data[0]);
  let nav = await utilities.getNav();
  const className =
 ...
 * ************************** */
invCont.getInventorybyIdToUpdate = async function (req, res, next) {
...
invCont.deleteInventory = async (req, res) => {
  ...
  console.log(inv_id);
  const del = await invModel.deleteInventoryByid(inv_id);
  if (del) {
